/* Includes ------------------------------------------------------------------*/
#include "wb32f10x.h"
#include <stdio.h>
#include <string.h>
#include "bsp_uart1.h"
#include "drv_spi_flash_quad.h"

/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define FLASH_WRITE_ADDRESS       0x1000
#define FLASH_READ_ADDRESS        FLASH_WRITE_ADDRESS
#define FLASH_SECTOR_TO_ERASE     FLASH_WRITE_ADDRESS

/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
const uint8_t WriteData[] = 
{
  ,
};
uint8_t ReadBuffer[sizeof(WriteData)];
uint32_t jedec_id;
uint16_t mfr_did;

/* Private function prototypes -----------------------------------------------*/
/* Private functions ---------------------------------------------------------*/

int main(void)
{
  uint32_t tmp;

  /* Init UART1 to 115200-8n1 for print message */
  uart1_init(96000000, 115200);

  /* Initialize the SPI FLASH driver */
  SpiFlash_Init();

  printf("Quad SPI Flash example\r\n");

  /* Enable QE bit */
  tmp = SpiFlash_ReadStatusReg2();
  printf("StatusReg2 = 0x%02X\r\n", tmp);
  if (!(tmp & 0x02)) {
    printf("QE bit is not set!\r\n");
    printf("Set QE bit...\r\n");
    tmp |= 0x02;
    SpiFlash_WriteStatusReg2(tmp);
    tmp = SpiFlash_ReadStatusReg2();
    printf("StatusReg2 = 0x%02X\r\n", tmp);
  }

  /* Get SPI Flash ID */
  jedec_id = SpiFlash_ReadIdentification();
  printf("This flash chip JEDEC ID is 0x%06X\r\n", jedec_id);

  /* Get SPI Flash ID through Quad I/O */
  mfr_did = SpiFlash_QuadReadMidDid();
  printf("This flash chip Manufacture/Device ID is 0x%04X\r\n", mfr_did);

  /* Perform a write in the Flash followed by a read of the written data */
  /* Erase SPI FLASH Sector to write on */
  SpiFlash_SectorErase(FLASH_SECTOR_TO_ERASE);

  /* Write WriteData data to SPI FLASH memory through Quad I/O */
  SpiFlash_QuadWriteBuffer(FLASH_WRITE_ADDRESS, WriteData, sizeof(WriteData));

  /* Read data from SPI FLASH memory through Quad I/O */
  SpiFlash_QuadFastRead(FLASH_READ_ADDRESS, ReadBuffer, sizeof(ReadBuffer));

  /* Check the correctness of written data */
  if(memcmp(WriteData, ReadBuffer, sizeof(WriteData)) == 0)
  {
    printf("Flash progam and read operation ok.\r\n");
  }
  else
  {
    printf("Flash progam and read operation failed!\r\n");
  }

  /* Infinite loop */
  while (1)
  {
  }
}

#ifdef  USE_FULL_ASSERT
/**
  * @brief  Reports the name of the source file and the source line number
  *         where the assert_param error has occurred.
  * @param  file: pointer to the source file name
  * @param  line: assert_param error line source number
  * @retval None
  */
void assert_failed(uint8_t* file, uint32_t line)
{ 
  /* User can add his own implementation to report the file name and line number,
     ex: printf("Wrong parameters value: file %s on line %d\r\n", file, line) */

  /* Infinite loop */
  while (1)
  {
  }
}
#endif
