...
int main(void)
{
  ...
  printf("[SPIS1_Slave_DMA]\r\n");

  /* Enable the GPIOA and SPIS1 clock */
  RCC_APB1PeriphClockCmd(RCC_APB1Periph_BMX1 |RCC_APB1Periph_GPIOA |RCC_APB1Periph_SPIS1, ENABLE);
  RCC_SPIS1CLKCmd(ENABLE);
  ...
  /*
    ...
    PA7 (SPIS1_SI)
  */
  GPIO_Init(GPIOA, GPIO_Pin_4 |GPIO_Pin_5 |GPIO_Pin_6 |GPIO_Pin_7, GPIO_MODE_AF |GPIO_OTYPE_PP |GPIO_PUPD_NOPULL |GPIO_SPEED_HIGH |GPIO_AF6);
  ...
  SPI_Init(SPIS1, &SPI_InitStructure);
  SPI_ITConfig(SPIS1, 0xFF, DISABLE);
  SPI_DMATxReqLevelConfig(SPIS1, 2);  /* Must adhere to this equation: DMAC.CTLLx.DEST_MSIZE <= SPI.FIFO_DEPTH - SPI.DMATDLR */
  SPI_DMARxReqLevelConfig(SPIS1, 0);  /* Must adhere to this equation: DMAC.CTLLx.SRC_MSIZE = SPI.DMARDLR + 1 */
  ...
  DMAC_Channel_InitStruct.DMAC_DestinationBaseAddr = (uint32_t)&SPIS1->DR;
  ...
  DMAC_Channel_InitStruct.DMAC_DestinationHardwareHandshakingInterfaceAssign = DMAC_HardwareHandshakingInterface_SPIS1_TX;
  DMAC_Channel_InitStruct.DMAC_MaximumAMBABurstLength = 0;
  DMAC_Channel_Init(DMAC1, DMAC_Channel_0, &DMAC_Channel_InitStruct);

  DMAC_Channel_InitStruct.DMAC_SourceBaseAddr = (uint32_t)&SPIS1->DR;
  ...
  DMAC_Channel_InitStruct.DMAC_ChannelPriority = 0;
  DMAC_Channel_InitStruct.DMAC_ProtectionControl = 0x1;
  DMAC_Channel_InitStruct.DMAC_SourceHardwareHandshakingInterfaceAssign = DMAC_HardwareHandshakingInterface_SPIS1_RX;
  ...
  while(SPI_GetFlagStatus(SPIS1, SPI_FLAG_BUSY) != RESET);
