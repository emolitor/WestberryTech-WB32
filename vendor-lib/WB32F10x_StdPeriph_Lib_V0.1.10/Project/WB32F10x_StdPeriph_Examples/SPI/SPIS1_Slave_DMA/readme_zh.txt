...
本例程与QSPI_Master_DMA例程演示如何以DMA的方式完成SPI主机和SPI从机之间的通信。
...
开发板A需加载QSPI_Master_DMA程序，开发板B需加载SPIS1_Slave_DMA程序。
...
在QSPI_Master_DMA程序中，QSPI外设被配置为SPI主机，使用DMA发送和接收数据，通信速率被配置为 12 Mbit/s。 \n
在SPIS1_Slave_DMA程序中，SPIS1外设被配置为SPI从机，使用DMA发送和接收数据。
...
@par 目录内容 
  ...
  - SPI/SPIS1_Slave_DMA/SPIS1_Slave_DMA.uvprojx   MDK5工程文件
  - SPI/SPIS1_Slave_DMA/SPIS1_Slave_DMA.uvoptx    MDK5工程配置文件
  - SPI/SPIS1_Slave_DMA/JLinkSettings.ini         Jlink配置文件
  - SPI/SPIS1_Slave_DMA/main.c                    主程序
...
为了使程序工作，您必须执行以下操作 :
 - 使用 MDK5 打开此例程文件夹中的 SPIS1_Slave_DMA.uvprojx 工程
 - 重新编译该工程并下载到目标存储器中
 - 运行此例程
