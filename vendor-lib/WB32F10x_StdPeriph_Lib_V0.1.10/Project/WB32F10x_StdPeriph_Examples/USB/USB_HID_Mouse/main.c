...
int mouse_send_report(void)
{
  ...
  if (UsbdCoreInfo.DeviceState == USBD_STATE_CONFIGURED)
  {
    ...
    {
      USBD_HW_Transmit(0x81, hid_report_buf, 4);
...
int main(void)
{
  ...
  while (1)
  {
    ...
    {
      /* Clear x-axis, y-axis and wheel buffer */
      hid_report_buf[1] = hid_report_buf[2] = hid_report_buf[3] = 0;

      if (key1_event == 1)
        hid_report_buf[1] = 10;       // x-axis + 10

      if (key2_event == 1)
        hid_report_buf[0] |= 0x01;    // Left Mouse Button down
      else if (key2_event == 2)
        hid_report_buf[0] &= ~0x01;   // Left Mouse Button up

      while (mouse_send_report() == 1);
    }
